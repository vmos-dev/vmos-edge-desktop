...
function parseQueryString(query: string | undefined): Record<string, string | undefined> {
...
function parseQueryStringNormalized(query: string | undefined): Record<string, string | undefined> {
...
function parsePortStrict(value: string | number | null | undefined): number | undefined {
...
const IP_VERSIONS = ['dual', 'ipv4', 'ipv6', 'ipv4-prefer', 'ipv6-prefer'] as const

function parseIpVersion(value: string | undefined): (typeof IP_VERSIONS)[number] {
...
function parseUrlLike(
  ...
  return options.requireAuth ? ({ ...result, auth } as UrlLikeParts & { auth: string }) : result
}
...
function getCipher(value: unknown): CipherType {
  ...
  return KNOWN_CIPHERS.has(aliased as CipherType) ? (aliased as CipherType) : 'auto'
}
...
function parseVmessQuantumult(content: string): IProxyVmessConfig {
  ...
  const proxy: IProxyVmessConfig = {
    ...
    'skip-cert-verify':
      params['tls-verification'] === undefined ? undefined : !parseBool(params['tls-verification'])
  }
    ...
    switch (proxy.network) {
      ...
      case 'http': {
        const hosts = Array.isArray(transportHost)
          ? transportHost.map((h: any) => String(h).trim()).filter((h: string) => h)
          : hostFirst
            ? [hostFirst]
            : undefined

        let paths = Array.isArray(transportPath)
          ? transportPath.map((p: any) => String(p).trim()).filter((p: string) => p)
          : pathFirst
            ? [pathFirst]
            : []
...
function URI_VLESS(line: string): IProxyVlessConfig {
  ...
  const parseVlessRest = (
    ...
    const port = parseRequiredPort(parsed.port, 'Invalid vless uri: invalid port')
    return {
      uuidRaw: parsed.auth,
      server: parsed.host,
      port,
...
function URI_Trojan(line: string): IProxyTrojanConfig {
  ...
  if (params['client-fingerprint']) {
    proxy['client-fingerprint'] = params['client-fingerprint'] as ClientFingerprint
  }
...
function URI_Hysteria2(line: string): IProxyHysteria2Config {
  const afterScheme = stripUriScheme(line, ['hysteria2', 'hy2'], 'Invalid hysteria2 uri')
  if (!afterScheme) {
...
function URI_Hysteria(line: string): IProxyHysteriaConfig {
  const afterScheme = stripUriScheme(line, ['hysteria', 'hy'], 'Invalid hysteria uri')
  if (!afterScheme) {
...
function URI_Wireguard(line: string): IProxyWireguardConfig {
  const afterScheme = stripUriScheme(line, ['wireguard', 'wg'], 'Invalid wireguard uri')
  if (!afterScheme) {
...
function URI_HTTP(line: string): IProxyHttpConfig {
  const afterScheme = stripUriScheme(line, ['http', 'https'], 'Invalid http uri')
  if (!afterScheme) {
...
function URI_SOCKS(line: string): IProxySocks5Config {
  const afterScheme = stripUriScheme(line, ['socks5', 'socks'], 'Invalid socks uri')
  if (!afterScheme) {
