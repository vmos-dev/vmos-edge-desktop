...
export class WebRtcPublisher {
  ...
  private config: Required<Omit<WebRtcConfig, 'videoMaxBitrate' | 'videoCodec'>> & { videoMaxBitrate?: number; videoCodec: VideoCodec };
  ...
  private async handleConnectionFailure(error: Error) {
    ...
    if (this._state !== 'idle' && this.retryCount < this.config.maxRetries) {
      ...
      console.log(`[WebRTC] Retrying in ${delay}ms (${this.retryCount}/${this.config.maxRetries})...`);
      ...
      // 彻底失败
      this.setState('error');
      this.events.onError?.(new Error(`Fatal: Connection failed after ${this.config.maxRetries} attempts. Reason: ${error.message}`));
      this.cleanup();
    }
  }
  ...
  private autoConfigureBitrate(stream: MediaStream) {
    ...
    // 简单的码率阶梯策略
    if (height <= 360) this.config.videoMaxBitrate = 800;       // 360p: 800kbps
    else if (height <= 480) this.config.videoMaxBitrate = 1200; // 480p: 1.2Mbps
    else if (height <= 720) this.config.videoMaxBitrate = 2000; // 720p: 2.0Mbps
    else this.config.videoMaxBitrate = 3500;                    // 1080p+: 3.5Mbps

    console.log(`[WebRTC] Auto-configured bitrate: ${this.config.videoMaxBitrate}kbps for ${height}p`);
  }
}