      ...
      >
        <div class="aic-tool-entry-header" @click.stop="$emit('toggle-detail', ti)">
          <svg
            ...
            class="aic-tool-status-icon is-loading"
            viewBox="0 0 1024 1024"
            width="13"
            height="13"
          >
            <path
              d="M512 64c-247.4 0-448 200.6-448 448s200.6 448 448 448 448-200.6 448-448-200.6-448-448-448zm0 832c-212.1 0-384-171.9-384-384S299.9 128 512 128s384 171.9 384 384-171.9 384-384 384z"
              fill="currentColor"
              opacity="0.3"
            />
            <path
              d="M512 128V64C264.6 64 64 264.6 64 512h64c0-212.1 171.9-384 384-384z"
              fill="currentColor"
            />
          </svg>
          <svg
            v-else-if="tool.status === 'fail'"
            class="aic-tool-status-icon fail"
            viewBox="0 0 1024 1024"
            width="13"
            height="13"
          >
          ...
          <svg
            class="aic-collapse-arrow small"
            :class="{ expanded: expandedDetails[ti] }"
            viewBox="0 0 1024 1024"
            width="11"
            height="11"
          >
        ...
        <div class="aic-collapsible-content" :class="{ expanded: expandedDetails[ti] }">
          <div class="aic-collapsible-inner">
            ...
            <div v-if="tool.fullResult" class="aic-tool-detail-content aic-tool-detail-result">
              {{
                tool.fullResult.length > 1000
                  ? tool.fullResult.substring(0, 1000) + `... (${tool.fullResult.length} chars)`
                  : tool.fullResult
              }}
            </div>
          </div>
